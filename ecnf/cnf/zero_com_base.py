...
class FlatZeroCoMGaussian(distrax.Distribution):
    def __init__(self, dim: int, n_nodes: int) -> None:
        self.dim = dim
        self.n_nodes = n_nodes
        self.zero_com_gaussian = CentreGravityGaussian(dim=dim, n_nodes=n_nodes)

    def _sample_n(self, key: PRNGKey, n: int) -> Array:
        sample = self.zero_com_gaussian._sample_n(key=key, n=n)
        flat_sample = jnp.reshape(sample, (n, (self.dim*self.n_nodes)))
        return flat_sample

    def log_prob(self, value: Array) -> Array:
        leading_shape = value.shape[:-1]
        unflat_value = jnp.reshape(value, (*leading_shape, self.n_nodes, self.dim))
        return self.zero_com_gaussian.log_prob(unflat_value)
